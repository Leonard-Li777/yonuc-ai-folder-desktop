...
interface FileListProps {
  ...
  sortBy?: 'name' | 'size' | 'modified' | 'type' | 'smartName' | 'analysisStatus' | 'author' // 可选的排序字段（虚拟目录传入）
  sortOrder?: 'asc' | 'desc' // 可选的排序顺序（虚拟目录传入）
  disableClientSort?: boolean // 是否禁用客户端排序（虚拟目录已在后端排序）
  onSortChange?: (
    sortBy: 'name' | 'size' | 'modified' | 'type' | 'smartName' | 'analysisStatus' | 'author',
    sortOrder: 'asc' | 'desc'
  ) => void // 排序变化回调
...
export const FileList: React.FC<FileListProps> = ({
  ...
  }, [directories, files, expandedDirectories, sortBy, sortOrder, snapshot, disableClientSort])
  ...
  if (viewMode === 'list') {
    ...
    const handleHeaderClick = (
      column: 'name' | 'size' | 'modified' | 'type' | 'smartName' | 'analysisStatus' | 'author'
    ) => {
          ...
          <thead className="text-xs text-foreground/80 dark:text-foreground/80 bg-muted dark:bg-muted sticky top-0 shadow-sm z-10 border-b border-border/50 dark:border-border/50">
            <tr>
              ...
              {shouldShowField('language') && (
                <th className="p-2 font-medium whitespace-nowrap">{getFieldLabel('language')}</th>
              )}
              {showAnalysisStatus && (
                <th
