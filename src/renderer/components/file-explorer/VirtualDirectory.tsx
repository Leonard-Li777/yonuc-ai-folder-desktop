...
export const VirtualDirectory: React.FC<VirtualDirectoryProps> = ({
  ...
  const handleQuickOrganize = async (userInstruction?: string) => {
    ...
    try {
      ...
      // 注意：通过 IPC 传递的 options 对象必须是可序列化的，userInstruction 为 undefined 时不应包含在对象中
      // 使用 JSON.parse(JSON.stringify()) 确保对象完全可序列化，移除 undefined 属性
      ...
      // 深度克隆并移除所有 undefined 属性，确保 IPC 安全
  ...
  const handleFileSelect = (
    ...
    } else if (!isFromCheckbox && files.length === 0) {
      setSelectedItem(null)
      setShowDetailsPanel(true)
    }
      ...
      if (!isUnlocked) {
        try {
          ...
          if (count < 3) {
            ...
            // Count >= 3 but config says locked, so unlock it
    ...
    <div className="flex-1 flex flex-col bg-muted/10 overflow-hidden">
              ...
              <div className="border-b border-border dark:border-border px-4 py-2 flex items-center justify-between bg-muted/20">
                ...
                <div className="flex items-center space-x-2 text-foreground dark:text-foreground">
                  ...
                  <div className="relative group">
                    <button
                      className={cn(
                        ...
                        selectedTagsForVirtualDir.size > 0
                          ...
                          : 'text-foreground bg-card dark:bg-card border-input dark:border-input hover:bg-muted dark:hover:bg-muted cursor-not-allowed opacity-50 pointer-events-none'
                      )}
                    ...
                    {selectedTagsForVirtualDir.size === 0 && (
                      <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 hidden group-hover:block z-50 w-max px-2 py-1 text-xs text-popover-foreground bg-popover border border-border rounded shadow-md pointer-events-none">
                        {t('请先勾选左侧要生成虚拟目录的标签')}
                      </div>
                    )}
                  </div>
      ...
      {showConfirmOrganizeDialog && organizePreview && (
        <ConfirmOrganizeDialog
          organizeType={aiGeneratedStructure ? 'quickOrganize' : 'virtualDirectory'}
        ...
        onRefresh={async () => {
          ...
          try {
            ...
            if (countResult.success) {
              setInvitationCount(countResult.count)
