...
export const RealDirectory: React.FC<RealDirectoryProps> = ({
  ...
  const getParentPath = (currentPath: string): string => {
    ...
    if (pathParts.length === 0) {
      ...
      // 如果是Windows路径且包含盘符，确保盘符后面有斜杠
      if (parentParts.length === 1 && /^[A-Za-z]:$/.test(parentParts[0])) {
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      const filesToAdd = selectedFiles
                        ...
                        .filter(i => !!i.path)
                      if (filesToAdd.length > 0) {
                        await window.electronAPI.addToAnalysisQueueResolved(filesToAdd)
