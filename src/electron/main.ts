...
import { createCoreEngine, type ICoreEngine, fileAnalysisService } from '@yonuc/core-engine'
...
async function initializeFullServices(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      const aiTimeout = configService.getValue<number>('AI_REQUEST_TIMEOUT') ?? 60000
...
async function setupIPCHandlers(): Promise<void> {
  ...
  // 目录上下文分析
  ipcMain.handle('analyze-directory-context', async (event, dirPath: string) => {
    ...
    return await directoryContextService.analyzeDirectoryContext(dirPath, currentLanguage as LanguageCode)
  })
