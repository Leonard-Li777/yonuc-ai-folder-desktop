  ...
  async getAnalyzedFilesCount(workspaceDirectoryPath?: string): Promise<number> {
    try {
      ...
      if (workspaceDirectoryPath) {
        ...
        const pathPrefix = path.normalize(workspaceDirectoryPath).replace(/\\/g, '/')
    ...
    try {
      ...
      return files.map((file) => {
        ...
        if (workspaceDirectoryPath) {
          ...
          const normalizedMonitorPath = path.normalize(workspaceDirectoryPath).replace(/\\/g, '/');

          // 获取文件所在目录
          const fileDir = path.dirname(normalizedFilePath).replace(/\\/g, '/');
