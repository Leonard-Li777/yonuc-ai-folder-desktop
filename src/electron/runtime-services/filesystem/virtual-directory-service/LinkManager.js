...
export class LinkManager {
    ...
    async createVirtualDirectoryStructure(workspaceDirectoryPath, directory) {
        try {
            ...
            const allDirectories = this.db
                .prepare('SELECT id, filters FROM virtual_directories WHERE id != ?')
                .all(directory.id);
    ...
    async createHardLinkAtLevel(virtualDirPath, file, levelTags) {
        try {
            ...
            logger.warn(LogCategory.FILE_ORGANIZATION, '[虚拟目录] 添加链接文件失败:', error);
        }
    }
    /**
     * 创建硬链接，失败时降级为符号链接
    ...
    async deduplicateHardLinksFromBottom(virtualDirPath, selectedTags) {
        try {
                    ...
                    for (let parentLevel = 1; parentLevel < deepLevel; parentLevel++) {
                        ...
                        if (fs.existsSync(parentFilePath)) {
                            try {
                                ...
                                logger.warn(LogCategory.FILE_ORGANIZATION, '[虚拟目录] 清理父级目录重复链接失败:', error);
                            }
                        }
                    }
                }
            ...
            logger.warn(LogCategory.FILE_ORGANIZATION, '[虚拟目录] 清理冗余链接异常:', error);
        }
    }
    /**
     * 递归获取目录下所有文件
    ...
    getAllFilesInDirectory(dirPath) {
        ...
        try {
            ...
            logger.warn(LogCategory.FILE_ORGANIZATION, '[虚拟目录] 获取目录下所有文件失败:', error);
        }
        return files;
    }
    /**
    ...
    async cleanupFilesInOtherVirtualDirectories(virtualDirPath, currentFiles, currentTagChain, workspaceDirectoryPath) {
        try {
            const allDirectories = this.db
                .prepare('SELECT filters FROM virtual_directories WHERE workspace_id = (SELECT id FROM workspace_directories WHERE path = ?)')
                    ...
                    for (let level = 1; level <= otherTagChain.length; level++) {
                        ...
                        if (fs.existsSync(otherFilePath)) {
                            try {
                                ...
                                logger.warn(LogCategory.FILE_ORGANIZATION, '[虚拟目录] 清理其他虚拟目录重复链接失败:', error);
                            }
                        }
                    }
                }
            ...
            logger.warn(LogCategory.FILE_ORGANIZATION, '[虚拟目录] 清理其他虚拟目录失败:', error);
        }
    }
    /**
     * 递归清理空目录
    ...
    async cleanupVirtualDirectory(workspaceDirectoryPath) {
        try {
            ...
            logger.warn(LogCategory.FILE_ORGANIZATION, '[虚拟目录] 清理虚拟目录失败:', error);
        }
    }
    async cleanupDirectoryRecursive(dirPath, analyzedFileNames, analyzedFilePaths) {
        try {
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                    ...
                    try {
                        ...
                        logger.warn(LogCategory.FILE_ORGANIZATION, '[虚拟目录] 递归清理空目录失败:', error);
                    }
                }
                    ...
                    if (!analyzedFileNames.has(entry.name)) {
                        try {
                            ...
                            logger.warn(LogCategory.FILE_ORGANIZATION, '[虚拟目录] 递归清理未分析文件失败:', error);
                        }
                    }
                }
            }
            ...
            logger.warn(LogCategory.FILE_ORGANIZATION, '[虚拟目录] 递归清理目录异常:', dirPath, error);
        }
    }
    /**
     * 删除虚拟目录对应的物理 tag 目录链
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                    ...
                    try {
                        ...
                        logger.warn(LogCategory.FILE_ORGANIZATION, '[虚拟目录] 查找硬链接 stat 失败:', e);
                    }
                }
            }
        }
        catch (error) {
            logger.warn(LogCategory.FILE_ORGANIZATION, '[虚拟目录] 查找硬链接目录遍历失败:', error);
        }
        return null;
    }
    /**
