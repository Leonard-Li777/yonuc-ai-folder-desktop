import { LogCategory, logger, isExtensionDimension, isPanDimension, filterDimensionTags } from '@firefly/shared';
...
export class DimensionManager {
    db;
    _extensionMap = null;
    static _logicPanProjectionsCache = null;
    constructor(db) {
        this.db = db;
    }
    /**
            ...
            try {
                ...
                if (Array.isArray(items)) {
                    for (const item of items) {
                        if (!item.zh || !item.dimensionId || !item.logicPanDimension)
                            continue;
    ...
    buildExtensionMap() {
        ...
        const l3ExtDims = rawDimensions.filter(d => Number(d.level) === 3 && /扩展名|Extension/i.test(d.name));
     ...
     */
    collectTriggerChainTags(dimId, nameToIdMap, idToDim, visited = new Set()) {
     ...
     */
    getExtsForParentTag(parentTagValue) {
        const map = this.buildExtensionMap();
        return map.get(parentTagValue) || [];
    }
    async getDimensionGroups(options, _language) {
        // 兼容旧的 string 参数
        const opts = typeof options === 'string'
            ? { workspaceDirectoryPath: options, language: _language }
            : options || {};
        const { workspaceDirectoryPath, excludeExtensionDimension = false, removeEmptyTags = false, selectedTags = [], unionMode = 'union' } = opts;
        ...
        try {
            ...
            if (excludeExtensionDimension) {
                rawDimensions = rawDimensions.filter((d) => !isExtensionDimension(d));
            }
            // 创建名称到 ID 的映射
            const nameToIdMap = new Map();
            rawDimensions.forEach(d => {
            ...
            const dimsWithChildren = new Set();
            rawDimensions.forEach(d => {
                if (d.trigger_conditions) {
                    try {
                        const tcs = JSON.parse(d.trigger_conditions);
                        if (Array.isArray(tcs)) {
                            tcs.forEach(tc => {
                                const pid = nameToIdMap.get(tc.parentDimension?.trim());
                                if (pid)
                                    dimsWithChildren.add(pid);
                            });
                        }
                    }
                    catch {
      ...
      `;
        ...
        `;
            ...
            const allCounts = this.db.prepare(globalCountQuery).all(...globalCountParams);
            ...
            for (const dim of rawDimensions) {
                // 标签处理
                const configShowEmptyTags = ConfigOrchestrator.getInstance().getValue('SHOW_EMPTY_TAGS') ?? false;
                ...
                const tagSet = new Set(validExistingTagNames);
              ...
              `;
                ...
                if (dimCountsMap) {
                    const isCompositeDim = Array.isArray(triggerConditions) &&
                        triggerConditions.length > 1 &&
                        new Set(triggerConditions.map((tc) => tc.parentDimension)).size > 1;
                    if (isCompositeDim) {
              ...
              `)
                                .all(r.file_type, dim.id, r.tag_name, r.tag_name);
                            const parentTagNames = fileParentTags.map(t => t.name);
                            const allowed = parentTagNames.length === 0 ||
                ...
                const normalTags = [];
                const entityTagsByAnchor = new Map();
     ...
     */
    removeEmptyTagsRecursive(groups) {
        // 构建父维度 ID 到子维度的映射
        ...
        while (changed) {
            ...
            for (const group of groups) {
                ...
                group.tags = group.tags.filter(tag => {
                    ...
                    // 有子维度且该标签在子维度中有数据，保留
                    if (hasChildren) {
                        const childHasTag = children.some(child => child.tags.some(t => t.fileCount > 0 && t.dimensionId !== group.id));
                        if (childHasTag)
                            return true;
                    }
                    // 否则移除
    ...
    getFileTagsWithDimensions(fileId) {
        try {
            return this.db
                .prepare(`
