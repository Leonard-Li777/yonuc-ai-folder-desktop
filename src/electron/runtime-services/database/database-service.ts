...
export class DatabaseService {
  ...
  async addFile(file: FileInfo): Promise<string> {
    ...
    try {
      ...
      logger.info(LogCategory.DATABASE_SERVICE, '文件已添加到数据库', { filePath: file.path, fileHash })
      return fileHash
    } catch (error) {
  ...
  async findRootWorkspaceDirectory(filePath: string): Promise<WorkspaceDirectory | null> {
    ...
    try {
      ...
      for (const root of roots) {
        const normalizedRootPath = path.normalize(root.path).toLowerCase()
  ...
  async getFileByPath(filePath: string): Promise<any> {
    if (!this._db) throw new Error('数据库未初始化')
    try {
      const row = this._db.prepare(`SELECT * FROM files WHERE path = ?`).get(filePath) as any
      if (!row) return null
      return {
        id: row.id,
        name: row.name,
