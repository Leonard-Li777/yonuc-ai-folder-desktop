...
import { LogCategory, logger, getSharedSchemaName, getLanguageSchemaName, isTestEnvironment, ResourceLocator } from '@firefly/shared';
...
export class ConfigDbManager {
    ...
    appConfigMap = new Map();
    systemConfigMap = new Map();
    fileDimensionsCache = [];
    initialized = false;
     ...
     */
    async initialize(language = 'zh-CN', force = false) {
    ...
    loadFromJson = (db, language) => {
        try {
            ...
            logger.info(LogCategory.CONFIG, `ConfigDbManager: 从本地 JSON 加载初始配置完成 (语言: ${resolvedLanguage})`);
        }
        catch (error) {
     ...
     */
    loadInitialAppConfigToDb(db) {
        try {
            const region = __BUILD_REGION__.toLowerCase();
            const configPath = this.getConfigFilePath(`app-config.${region}.json`);
            if (!fs.existsSync(configPath)) {
                logger.warn(LogCategory.CONFIG, `ConfigDbManager: 初始 app-config 配置文件不存在: ${configPath}`);
                return;
            }
            const raw = fs.readFileSync(configPath, 'utf-8');
            const parsed = JSON.parse(raw);
            const insertStmt = db.prepare(`INSERT OR REPLACE INTO app_config (key, value, updated_at) VALUES (?, ?, ?)`);
            db.transaction(() => {
     ...
     */
    loadInitialSystemConfigToDb(db, language) {
        try {
            const insertStmt = db.prepare(`INSERT OR REPLACE INTO system_config (key, value, updated_at) VALUES (?, ?, ?)`);
            // 先清空表
            ...
            if (fs.existsSync(systemConfigPath)) {
                try {
                    ...
                    logger.error(LogCategory.CONFIG, `ConfigDbManager: 导入 system-config_${language}.json 失败:`, err);
                }
            }
            else {
                logger.warn(LogCategory.CONFIG, `ConfigDbManager: 初始 system-config 配置文件不存在: ${systemConfigPath}`);
            }
            ...
            if (fs.existsSync(localPresetPath)) {
                try {
                    ...
                    if (content && content.trim() !== '') {
                        ...
                        logger.info(LogCategory.CONFIG, `ConfigDbManager: 成功导入初始 LOCAL_MODEL_CONFIGS 数据`);
                    }
                }
                catch (err) {
                    logger.error(LogCategory.CONFIG, `ConfigDbManager: 导入 model_${language}.json 失败:`, err);
                }
            }
            else {
            ...
            if (fs.existsSync(ollamaPresetPath)) {
                try {
                    ...
                    if (content && content.trim() !== '') {
                        ...
                        logger.info(LogCategory.CONFIG, `ConfigDbManager: 成功导入初始 LOCAL_MODEL_CONFIGS_OLLAMA 数据`);
                    }
                }
                catch (err) {
                    logger.error(LogCategory.CONFIG, `ConfigDbManager: 导入 ollama_${language}.json 失败:`, err);
                }
            }
            else {
                logger.warn(LogCategory.CONFIG, `ConfigDbManager: Ollama 模型配置文件不存在: ${ollamaPresetPath}`);
            }
                ...
                try {
                    ...
                    if (content && content.trim() !== '') {
                        ...
                        if (Array.isArray(localPresets)) {
                            ...
                            const mappedPresets = localPresets.map((p) => ({ ...p, provider: p.id }));
                            insertStmt.run('CLOUD_MODEL_CONFIGS', JSON.stringify(mappedPresets), now);
                            logger.info(LogCategory.CONFIG, `ConfigDbManager: 成功导入初始 CLOUD_MODEL_CONFIGS 数据 (${mappedPresets.length} 个服务商)`);
                        }
                    }
                }
                catch (err) {
                    logger.error(LogCategory.CONFIG, `ConfigDbManager: 导入 providers_${language}.json 失败:`, err);
                }
            }
            else {
                logger.warn(LogCategory.CONFIG, `ConfigDbManager: 云端模型配置文件不存在: ${providersPresetPath}`);
            }
        }
        catch (error) {
     ...
     */
    loadInitialFileDimensionsToDb(db, language) {
        try {
            const filePath = ResourceLocator.resolveDimension(`fileDimension_${language}.json`);
            if (!fs.existsSync(filePath)) {
                logger.warn(LogCategory.CONFIG, `ConfigDbManager: fileDimension 配置文件不存在: ${filePath}`);
                return;
            }
      ...
      `);
            db.transaction(() => {
                for (const dim of dimensions) {
                    ...
                    insertStmt.run(dim.id, dim.name, dim.level, tags, triggerConditions, dim.is_ai_generated ? 1 : 0, dim.description || null, applicableFileTypes, contextHints, metadata, dim.created_at || new Date().toISOString());
                }
            })();
            ...
            logger.info(LogCategory.CONFIG, `ConfigDbManager: 成功导入 ${dimensions.length} 个 file_dimensions 数据`);
        }
        catch (error) {
     ...
     */
    loadAllConfigsFromDb(db) {
        this.appConfigMap.clear();
        this.systemConfigMap.clear();
        const appConfigs = db.prepare('SELECT key, value FROM app_config').all();
        ...
        const systemConfigs = db.prepare('SELECT key, value FROM system_config').all();
    ...
    async syncFromCloud() {
        ...
        const supabase = createSupabaseClient(WORKSPACE_CONSTANTS.SUPABASE_URL, WORKSPACE_CONSTANTS.SUPABASE_ANON_KEY, machineId, signature, this.currentLanguage);
        const db = databaseService.db;
        if (!db)
            return;
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                try {
                    ...
                    logger.warn(LogCategory.CONFIG, `ConfigDbManager: 拉取 system_config 失败: ${err.message}`);
                }
                ...
                try {
                    ...
                    logger.warn(LogCategory.CONFIG, `ConfigDbManager: 拉取 file_dimensions 失败: ${err.message}`);
                }
                // 4. 在事务中一次性写入（失败则回滚）
                db.transaction(() => {
                    // 合并导入 app_config（云端数据覆盖本地，保留本地独有 key）
                    if (appData.length > 0) {
                        const appInsert = db.prepare(`INSERT OR REPLACE INTO app_config (key, value, updated_at) VALUES (?, ?, ?)`);
                        const now = new Date().toISOString();
                        appData.forEach(row => {
                    ...
                    if (systemData.length > 0) {
                        const systemInsert = db.prepare(`INSERT OR REPLACE INTO system_config (key, value, updated_at) VALUES (?, ?, ?)`);
                        const now = new Date().toISOString();
                    ...
                    if (dimensionData.length > 0) {
                        if (this.currentLanguage !== 'zh-CN') {
                            const hasChinese = dimensionData.some(d => typeof d.name === 'string' && /[\u4e00-\u9fa5]/.test(d.name));
                            if (hasChinese) {
                                logger.warn(LogCategory.CONFIG, `ConfigDbManager: 云端拉取的维度包含中文，与当前语言 (${this.currentLanguage}) 不符，跳过覆盖`);
                                return;
                            }
                        }
                        db.prepare('DELETE FROM file_dimensions').run();
            ...
            `);
                        ...
                        dimensionData.forEach(dim => {
                            const tags = typeof dim.tags === 'string' ? dim.tags : JSON.stringify(dim.tags || []);
                            const trigger_conditions = typeof dim.trigger_conditions === 'string'
                                ? dim.trigger_conditions
                                : dim.trigger_conditions
                                    ? JSON.stringify(dim.trigger_conditions)
                                    : null;
                            const applicable_file_types = typeof dim.applicable_file_types === 'string'
                                ? dim.applicable_file_types
                                : dim.applicable_file_types
                                    ? JSON.stringify(dim.applicable_file_types)
                                    : null;
                            const context_hints = typeof dim.context_hints === 'string'
                                ? dim.context_hints
                                : dim.context_hints
                                    ? JSON.stringify(dim.context_hints)
                                    : null;
                            const metadata = dim.metadata ? JSON.stringify(dim.metadata) : null;
                            dimInsert.run(dim.id, dim.name, dim.level, tags, trigger_conditions, dim.is_ai_generated ? 1 : 0, dim.description || null, applicable_file_types, context_hints, metadata, dim.created_at || now);
                        });
                    }
                })();
                // 4. 重新加载至内存并广播
                ...
                lastError = error;
                logger.warn(LogCategory.CONFIG, `ConfigDbManager: 云端同步失败 (尝试 ${attempt}/${maxRetries}): ${error.message}`);
     ...
     */
    getAppValue(key) {
        const cached = this.appConfigMap.get(key.toUpperCase());
        if (cached !== undefined)
            return cached;
    ...
    getSystemValue(key) {
        return this.systemConfigMap.get(key.toUpperCase());
    }
    getAllAppConfig() {
    ...
    setAppValue(key, value) {
        ...
        try {
            db.prepare(`INSERT OR REPLACE INTO app_config (key, value, updated_at) VALUES (?, ?, ?)`).run(keyUpper, JSON.stringify(value), new Date().toISOString());
            this.broadcastConfigUpdate();
        }
        catch (err) {
    ...
    setSystemValue(key, value) {
        ...
        try {
            db.prepare(`INSERT OR REPLACE INTO system_config (key, value, updated_at) VALUES (?, ?, ?)`).run(keyUpper, JSON.stringify(value), new Date().toISOString());
            this.broadcastConfigUpdate();
        }
        catch (err) {
...
//# sourceMappingURL=config-db-manager.js.map