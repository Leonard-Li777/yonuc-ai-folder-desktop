...
export class AnalysisQueueService {
  ...
  async start(): Promise<void> {
    ...
    while (this.running) {
      try {
        ...
        if (!next) {
          // logger.debug(LogCategory.ANALYSIS_QUEUE, '[分析队列] 无待处理项目，进入等待')
        ...
        this.current = next
        logger.info(LogCategory.ANALYSIS_QUEUE, `[分析队列] 开始处理项目: ${next.path} (${next.type})`)
  ...
  private emitUpdate(): void {
    ...
    logger.info(LogCategory.ANALYSIS_QUEUE, '[分析队列] 发送状态更新', { running: snapshot.running, itemCount: snapshot.items.length })
    windows.forEach(win => {
  ...
  private async processFile(item: AnalysisQueueItem): Promise<void> {
    try {
      ...
      const existingFile = db.prepare(`
        ...
        WHERE path = ? 
        AND workspace_id = ?
      `).get(item.path, workspaceId) as { id: string } | undefined
  ...
  private async saveCloudResultToDB(item: AnalysisQueueItem, workspaceId: number, fileHash: string, data: any): Promise<void> {
    try {
      ...
      logger.info(LogCategory.ANALYSIS_QUEUE, '[分析队列] 保存云端缓存结果到数据库', {
        filePath: item.path,
        fileId: fileId,
      ...
      const verifyFile = db.prepare(`
        SELECT id, is_analyzed FROM files 
        WHERE path = ? 
        AND workspace_id = ?
      `).get(item.path, workspaceId) as { id: string; is_analyzed: number } | undefined
