import { LogCategory, logger, FileCategory, isCategory, cleanSmartName, sanitizeAITagValue, isValidAITag } from '@firefly/shared';
 ...
 * 保存云端分析结果到数据库
 */
export async function saveCloudResult(item, fileFingerprint, data, isCloudCache, workspaceId, getModelName) {
    ...
    try {
        ...
        if (fileType &&
            ['jpg', 'jpeg', 'png', 'webp', 'mp4', 'mov', 'avi', 'mkv', 'webm', 'pdf', 'pdfx'].includes(fileType.toLowerCase())) {
            try {
                ...
                if (rootDir && rootDir.path) {
                    ...
                    if (isCategory(filePath, FileCategory.IMAGE)) {
                        ...
                        if (!nativeImageExtensions.includes(ext)) {
                            await thumbnailService.getOrGenerateOriginalTranscodedImage(filePath, fileFingerprint, item.name || path.basename(filePath), rootDir.path);
                        }
                    }
                }
            }
            catch (e) {
                logger.debug(LogCategory.ANALYSIS_QUEUE, '[分析队列] 保存云端结果时生成缩略图或预转码大图失败', e);
            }
        }
        ...
        const smartName = await databaseService.resolveUniqueSmartName(rawSmartName, fileFingerprint, workspaceId);
      ...
      `).run(fileFingerprint, smartName, description, stats.size, fileType, data.category ? JSON.stringify(data.category) : null, data.author || null, data.language || null, isHit, lastHitAt, fileData.syncStatus, new Date(stats.birthtime).toISOString(), new Date(stats.mtime).toISOString(), new Date(stats.atime).toISOString());
            db.prepare(`
      ...
      `).run(fileFingerprint, content, multimodalContent, data.lrc || null, fileData.metadata, fileData.analysisStats, qualityScore, fileData.qualityConfidence, fileData.qualityReasoning, fileData.qualityCriteria, fileData.groupingReason, fileData.groupingConfidence);
            db.prepare(`
        ...
        ON CONFLICT(workspace_id, path) DO UPDATE SET
          ...
          is_analyzed = 1,
          last_analyzed_at = ?
      `).run(fileFingerprint, workspaceId, directoryId, filePath, path.basename(filePath), 1, new Date().toISOString(), new Date(stats.birthtime).toISOString(), new Date(stats.mtime).toISOString(), new Date(stats.atime).toISOString(), new Date().toISOString());
            if (data.tags && Array.isArray(data.tags)) {
                // 预取所有正式维度名及预设标签，用于过滤和路由校验
                const allDimRows = db.prepare('SELECT id, name, tags FROM file_dimensions').all();
                ...
                let panDimensionIds = [4, 28];
                try {
                    panDimensionIds = ConfigOrchestrator.getInstance().getValue('PAN_DIMENSION_IDS') || [4, 28];
                }
                catch {
                ...
                for (const tag of data.tags) {
                    ...
                    if (!cleanName || !isValidAITag(cleanName, officialDimNames))
                        continue;
                    try {
                        ...
                        if (dimInfo) {
                            if (panIdSet.has(cloudDimId) ||
                                ...
                                // 检查是否命中其他非泛维度的预设标签
                                let foundOtherDimId = null;
                                for (const [otherId, otherInfo] of dimMap) {
                                    if (panIdSet.has(otherId) ||
                                        otherInfo.name === '作者' ||
                                        otherInfo.name === '内容标签')
                                        continue;
                        ...
                        let tagRow = db
                            ...
                            .get(cleanName, localDimId);
                        if (!tagRow) {
                            db.prepare(`INSERT INTO file_tags (name, dimension_id, sync_status, created_at) VALUES (?, ?, 0, ?)`).run(cleanName, localDimId, new Date().toISOString());
                            tagRow = db
                                .prepare('SELECT id FROM file_tags WHERE name = ? AND dimension_id = ?')
                                .get(cleanName, localDimId);
                        }
...
//# sourceMappingURL=save-cloud-result.js.map