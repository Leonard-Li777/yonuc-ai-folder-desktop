import { LogCategory, logger, FileCategory, isCategory } from '@firefly/shared';
import { t } from '@app/languages';
import { extractPureLyrics } from '@firefly/core-engine';
import { databaseService } from '../../database/database-service';
import path from 'node:path';
/**
 * 处理本地 AI 分析任务（质量评分 + 维度标签）
 */
export async function processLocalAnalysis(item, fileFingerprint, fileInfo, thumbnailRelativePath, rootWorkspaceDirPath, timer, deps, options, updateItemStatus, processNewDimensionSuggestions) {
    ...
    if (options.initialStage === 3 && !options.forceReanalyze) {
        logger.info(LogCategory.ANALYSIS_QUEUE, `[阶段复用] 已处于 stage 3 且非重新分析，复用已有质量数据，跳过 AI 质量评分: ${item.name}`);
      ...
      `)
        ...
        processResult = {
            ...
            qualityCriteria: (() => {
                if (!existingQuality.quality_criteria)
                    return undefined;
        ...
        processResult = deps.fileProcessor
            ? await deps.fileProcessor.processFileWithTimeout(fileFingerprint, fileInfo, thumbnailRelativePath
                ? path.join(rootWorkspaceDirPath, thumbnailRelativePath)
                : undefined, deps.errorRecoveryConfig.fileProcessingTimeout, isSpeedy)
            : {
    ...
    const dimResult = await deps.dimensionAnalyzer.analyzeFileWithDimensions(filePath, item.name, fileType, fileInfo.size, processResult.content || '', processResult.multimodalContent, processResult.qualityScore || 3, processResult.metadata, existingDimensions, directoryContext, magikaCategory, isSpeedy);
    if (dimResult) {
        const isAudio = fileType === 'audio' || isCategory(filePath || '', FileCategory.AUDIO);
        if (isAudio) {
            const metadataLyrics = getFallbackLyrics(processResult.metadata);
            ...
            `)
                    ...
                    logger.error(LogCategory.ANALYSIS_QUEUE, '[分析队列] 更新歌词/台词至数据库失败:', lrcDbErr);
                }
            }
        }
 ...
 * 从元数据中提取候选歌词
 */
function getFallbackLyrics(metadata) {
    if (!metadata)
        return null;
...
//# sourceMappingURL=process-local-analysis.js.map