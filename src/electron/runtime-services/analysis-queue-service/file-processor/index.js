import { LogCategory, logger, PerformanceTimer, FileCategory, getFileCategory, isCategory, saveAuthorTagsFromMetadata, saveLanguageTagsFromMetadata, saveMagikaGroupTag, saveExtensionTags, saveMagikaIsTextTag, getMagikaGroupFromExtension, BROWSER_NATIVE_IMAGE_EXTS, shouldSkipAIServiceInTest, applyMarkitdownBenchmark, calculateFileFingerprint, cleanSmartName, isGibberishOcrText } from '@firefly/shared';
import { ConfigOrchestrator } from '../../../config/config-orchestrator';
import { databaseService } from '../../database/database-service';
import { quotaChecker } from '../../user-tier/quota-checker-proxy';
import { getMimeType, TextFileProcessor, extractPureLyrics } from '@firefly/core-engine';
import { executeProPreflight, executeProTagReconciliation, executeProVisualTagging, executeProTextTagging } from '@pro';
import { omniService } from '../../system/omni-service';
import { thumbnailService } from '../../filesystem/thumbnail-service';
import { anydocService } from '../../system/anydoc-service';
...
export function getFileStageFromDB(db, workspaceId, filePath) {
    try {
        const row = db
            .prepare(`
 ...
 */
export function getFileAnalysisStateFromDB(db, workspaceId, filePath) {
    try {
        const row = db
            .prepare(`
...
export class FileProcessor {
    ...
    updateItemStatus;
    pause;
    collectAnalysisStats;
    getModelName;
    analyzeDirectoryContext;
    processNewDimensionSuggestions;
    mockData = null;
    selectWorkspaceFileStmt = null;
    insertFileStmt = null;
    insertWorkspaceFileStmt = null;
    deleteTagRelationsStmt = null;
    ...
    `);
      ...
      ON CONFLICT(workspace_id, path) DO UPDATE SET
        ...
        accessed_at = ?
    `);
        this.deleteTagRelationsStmt = db.prepare('DELETE FROM file_tag_relations WHERE file_fingerprint = ?');
    }
    constructor(getDependencies, updateItemStatus, pause, collectAnalysisStats, getModelName, analyzeDirectoryContext, processNewDimensionSuggestions) {
        this.getDependencies = getDependencies;
        this.updateItemStatus = updateItemStatus;
        this.pause = pause;
        this.collectAnalysisStats = collectAnalysisStats;
        this.getModelName = getModelName;
        this.analyzeDirectoryContext = analyzeDirectoryContext;
        this.processNewDimensionSuggestions = processNewDimensionSuggestions;
    }
    /**
     * 获取数据库中已存在的基础信息
     */
    getExistingBasicData(db, fingerprint, workspaceId, filePath) {
        const result = {};
        try {
            ...
            if (fileRow?.category) {
                try {
                    ...
                    logger.warn(LogCategory.ANALYSIS_QUEUE, `[复用数据] 解析 category JSON 失败: ${fingerprint}`);
                }
            }
            // 2. 获取 thumbnailPath (workspace_files 表)
            const wsFileRow = db
                .prepare('SELECT thumbnail_path FROM workspace_files WHERE workspace_id = ? AND path = ? AND file_fingerprint = ?')
                .get(workspaceId, filePath, fingerprint);
            ...
            if (contentRow) {
                if (contentRow.metadata) {
                    try {
                        ...
                        logger.warn(LogCategory.ANALYSIS_QUEUE, `[复用数据] 解析 metadata JSON 失败: ${fingerprint}`);
                    }
                }
     ...
     */
    filterMissingIndicators(indicators, existingBasicData, magikaCategory) {
     ...
     * 处理文件项
     */
    async processFile(item, signal, phase = 'all', cpuSkipped = false) {
        const deps = this.getDependencies();
        try {
            // V2.2 架构：优先从 item.path 获取，如果存在则通过 item_id 查询数据库
            let filePath = item.file_path || item.path;
            ...
            if (!filePath && itemId) {
                ...
                if (db) {
                    ...
                    if (itemType === 'directory') {
                        ...
                        wf = db.prepare(`SELECT path FROM workspace_files WHERE id = ?`).get(itemId);
                    }
                    if (wf && wf.path) {
                        filePath = wf.path;
                    }
            ...
            if (mockJsonPath && fs.existsSync(mockJsonPath)) {
                ...
                logger.warn(LogCategory.ANALYSIS_QUEUE, `[测试模式] 拦截器未能从模拟库找到结果: ${item.name}`);
                // 普通集成测试模式下未命中 mock 也直接标记为完成，避免因缺少真实 AI 服务导致队列卡死；E2E 测试模式正常走真实 AI
                const isTest = shouldSkipAIServiceInTest();
                if (isTest) {
                    logger.info(LogCategory.ANALYSIS_QUEUE, `[测试模式] 文件不在 mock 库中，标记为已跳过: ${item.name}`);
            ...
            const reuseBasicAnalysisData = ConfigOrchestrator.getInstance().getValue('REUSE_BASIC_ANALYSIS_DATA') ?? true;
            ...
            if (phase === 'all') {
                ...
                const forcedReextract = item.forceReanalyze === true && !reuseBasicAnalysisData && alreadyAnalyzed;
                if (initialStage >= 2 && !forcedReextract) {
                    if (analysisMode === 'full' || analysisMode === 'quick_name') {
                        ...
                        logger.info(LogCategory.ANALYSIS_QUEUE, `[串行队列] 文件已处于 Stage ${initialStage} >= 2，跳过 CPU 提取，直接进入 GPU AI 阶段: ${item.name}`);
                        return this.processFile(item, signal, 'gpu', true);
                    }
                    // 非 AI 模式（simple/document）不执行 AI 阶段（stage3/4），
                    // 继续走 CPU 提取流程，复用已有数据后在基础分析分支完成
                    logger.info(LogCategory.ANALYSIS_QUEUE, `[串行队列] 文件已处于 Stage ${initialStage} >= 2，非 AI 模式（${analysisMode}）跳过 AI 阶段: ${item.name}`);
                }
            }
            ...
            logger.info(LogCategory.ANALYSIS_QUEUE, `[配额调试] 文件: ${filePath}, workspaceId: ${currentWorkspaceId}, 目录类型: ${rootWorkspaceDir?.type || 'unknown'}, isPrivate: ${isPrivate}`);
            if (isPrivate) {
                try {
            ...
            if (phase === 'gpu') {
                ...
                if (analysisMode !== 'full' && analysisMode !== 'quick_name') {
                    logger.info(LogCategory.ANALYSIS_QUEUE, `[分析队列] 非 AI 模式（${analysisMode}）跳过 GPU AI 阶段: ${item.name}`);
                    this.updateItemStatus(item.id, 'completed', 100);
                    return;
                }
                const existingWorkspaceFile = this.selectWorkspaceFileStmt.get(currentWorkspaceId, filePath);
                let fileFingerprint = existingWorkspaceFile?.file_fingerprint;
                if (!fileFingerprint) {
                    logger.warn(LogCategory.ANALYSIS_QUEUE, `[分析队列] GPU 阶段发现文件尚未记录指纹 (${filePath})，正在自动计算并补建基础记录...`);
                    try {
                        fileFingerprint = await calculateFileFingerprint(filePath);
                        const stats = currentStats || fs.statSync(filePath);
                        const { fileType: initialFileType, smartName: initialSmartName } = this.getEnhancedFileInfo(path.basename(filePath), item.type, filePath, null);
                        // 确保 files 与 workspace_files 正确关联
                        db.prepare(`INSERT INTO files (file_fingerprint, smart_name, size, type, created_at, modified_at, accessed_at)
               VALUES (?, ?, ?, ?, ?, ?, ?)
               ON CONFLICT(file_fingerprint) DO UPDATE SET
                 smart_name = COALESCE(files.smart_name, excluded.smart_name)`).run(fileFingerprint, initialSmartName, stats.size, initialFileType, new Date(stats.birthtime).toISOString(), new Date(stats.mtime).toISOString(), new Date(stats.atime).toISOString());
                        db.prepare(`INSERT INTO workspace_files (workspace_id, directory_id, path, name, file_fingerprint, is_analyzed, created_at, modified_at, accessed_at)
               VALUES (?, (SELECT id FROM workspace_directories WHERE workspace_id = ? AND path = ?), ?, ?, ?, 0, ?, ?, ?)
               ON CONFLICT(workspace_id, path) DO UPDATE SET
                 file_fingerprint = excluded.file_fingerprint,
                 modified_at = excluded.modified_at,
                 accessed_at = excluded.accessed_at`).run(currentWorkspaceId, currentWorkspaceId, path.dirname(filePath), filePath, path.basename(filePath), fileFingerprint, new Date().toISOString(), new Date().toISOString(), new Date().toISOString());
                    }
                    catch (fpErr) {
                        logger.warn(LogCategory.ANALYSIS_QUEUE, `[分析队列] GPU 阶段自动计算指纹失败，平滑回退至 CPU 阶段提取: ${fpErr.message}`);
                ...
                const existingBasicData = this.getExistingBasicData(db, fileFingerprint, currentWorkspaceId, filePath);
                const enhancedInfo = this.getEnhancedFileInfo(item.name, item.type, filePath, existingBasicData.category || null);
                let baseMetadata = existingBasicData.metadata || {};
                // 健壮性保障：如果数据库现有 metadata 为空或缺乏 Exif 详细属性（如历史只存了 raw_smart_name 等），通过 Omni 补捞
                const hasExifDetail = Boolean(baseMetadata.Make ||
                    baseMetadata.Model ||
                ...
                try {
                    ...
                    logger.info(LogCategory.ANALYSIS_QUEUE, `[目录上下文] 文件 ${item.name} 开始 AI 分析，优先获取/自动分析所在父级目录: ${parentDir}`);
                    directoryContext = await this.analyzeDirectoryContext(parentDir, false);
                }
                catch (dirCtxError) {
                    logger.warn(LogCategory.ANALYSIS_QUEUE, `[目录上下文] GPU阶段预获取父级目录上下文失败: ${filePath}`, dirCtxError);
                }
                ...
                if (analysisMode === 'quick_name') {
                    const quickRes = await processQuickNameAnalysis(item, fileFingerprint, fileInfo, thumbnailRelativePath, rootWorkspaceDir.path, timer, deps, {
                        language,
                        directoryContext,
                    ...
                    }, this.updateItemStatus.bind(this), this.processNewDimensionSuggestions.bind(this));
                    processResult = quickRes.processResult;
                    dimResult = quickRes.dimResult;
                }
                else {
                    const fullRes = await processLocalAnalysis(item, fileFingerprint, fileInfo, thumbnailRelativePath, rootWorkspaceDir.path, timer, deps, {
                        language,
                        directoryContext,
                        magikaCategory: existingBasicData.category || null,
                        isSpeedy,
                ...
                try {
                    ...
                    const effectiveDirConfig = (await directoryContextService?.getEffectiveDirectoryConfig(parentDir)) ||
                        directoryContext;
                    ...
                    if (template) {
                        const fileRenameContext = {
                            ...
                            tags: dimResult?.tags || [],
                            dimensionTags: dimResult?.dimensionTags || (dimResult?.dimensions ? dimResult.dimensions : {}),
                    ...
                    logger.warn(LogCategory.ANALYSIS_QUEUE, `[智能命名模板] 渲染命名模板失败: ${filePath}`, templateErr);
                }
                ...
                // 保存本地分析结果
                const { workspaceFile } = await saveLocalAnalysisResult(item, fileFingerprint, processResult, existingBasicData.category || null, finalSmartName, enhancedInfo.fileType, thumbnailRelativePath, currentWorkspaceId, timer, this.collectAnalysisStats.bind(this), false, dimResult?.groupingReason, dimResult?.groupingConfidence, undefined, // markitdownBenchmark
                analysisMode === 'quick_name' ? 3 : 4, cpuSkipped);
            ...
            if (needsNewHash) {
                logger.info(LogCategory.ANALYSIS_QUEUE, `[分析队列] 准备计算真实文件指纹: ${item.name}${isTempHash ? ' (替换临时ID)' : ''}${metadataMismatched ? ' (元数据已变动)' : ''}`);
                // 1. 哈希计算：文件标识基石，3s 超时防护
                const tFpStart = Date.now();
                try {
                    fileFingerprint = await Promise.race([
                        calculateFileFingerprint(filePath),
                        new Promise((_, reject) => setTimeout(() => reject(new Error('文件哈希计算超时(3s)，文件可能损坏或被占用')), 3000))
                    ]);
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?)
             ON CONFLICT(file_fingerprint) DO UPDATE SET
               smart_name = COALESCE(files.smart_name, excluded.smart_name)`).run(fileFingerprint, initialSmartName, currentStats.size, initialFileType, new Date(currentStats.birthtime).toISOString(), new Date(currentStats.mtime).toISOString(), new Date(currentStats.atime).toISOString());
                    // 2. 确保 file_contents 表中有对应指纹记录
                    db.prepare(`INSERT OR IGNORE INTO file_contents (file_fingerprint) VALUES (?)`).run(fileFingerprint);
                    // 3. 更新物理文件关联的真实指纹
                    db.prepare(`UPDATE workspace_files SET 
              file_fingerprint = ?, 
              modified_at = ?,
              accessed_at = ?
            WHERE path = ?`).run(fileFingerprint, new Date().toISOString(), new Date().toISOString(), filePath);
                    logger.info(LogCategory.ANALYSIS_QUEUE, `[分析队列] 阶段1完成，已更新指纹与基础信息: ${item.name} (${fileFingerprint.substring(0, 8)}...), 耗时: ${stage1FingerprintMs}ms`);
                }
                catch (updateError) {
                ...
                logger.info(LogCategory.ANALYSIS_QUEUE, `[分析队列] 阶段1完成，复用已有指纹与基础信息: ${item.name} (${fileFingerprint.substring(0, 8)}...), 耗时: ${stage1LocalReuseMs}ms`);
            }
            ...
            if (!isPrivate) {
                ...
                const shouldSkipCache = (item.forceReanalyze === true && isLocallyAnalyzed) || analysisMode !== 'full';
            ...
            if (cloudCachedData) {
                ...
                try {
                    await this.saveCloudResultToDB(item, fileFingerprint, cloudCachedData, isCloudCache, currentWorkspaceId);
            ...
            if (!magikaCategory) {
                if (existingBasicData.category) {
                    ...
                    logger.info(LogCategory.ANALYSIS_QUEUE, `[复用数据] 已复用文件类型 (Magika): ${item.name}`);
                }
            ...
            let { fileType: enhancedFileType, smartName: enhancedSmartName } = this.getEnhancedFileInfo(item.name, item.type, filePath, magikaCategory);
            let contentResult = { content: '', metadata: {} };
            let thumbnailRelativePath = undefined;
            let directoryContext = null;
            // MarkitdownServer 提取阶段细分耗时（来自响应 time_ms/benchmark）
            let markitdownBenchmark = undefined;
            const extractPages = ConfigOrchestrator.getInstance().getValue('EXTRACT_PAGES') ?? 2;
            const maxDocOcrItems = ConfigOrchestrator.getInstance().getValue('MAX_DOCUMENT_OCR_ITEMS') ?? 0;
            const ocrModelSize = ConfigOrchestrator.getInstance().getValue('OCR_MODEL_SIZE') ?? 'tiny';
            const maxContentSizeKb = ConfigOrchestrator.getInstance().getValue('MAX_CONTENT_SIZE_KB') ?? 30;
            ...
            if (!effectiveExt && magikaCategory) {
                try {
                    const catObj = typeof magikaCategory === 'string' ? JSON.parse(magikaCategory) : magikaCategory;
            ...
            const effectiveVirtualPath = effectiveExt && !filePath.toLowerCase().endsWith(effectiveExt)
            ...
            try {
                ...
                if (analysisMode === 'simple') {
                    ...
                    directoryContext = await this.analyzeDirectoryContext(parentDir, false, true /* cacheOnly */);
                }
                else {
                ...
                if (directoryContext) {
                    ...
                    logger.info(LogCategory.ANALYSIS_QUEUE, `[目录上下文] 目录上下文暂不可用（将跳过目录维度增强）: ${parentDir}`);
                }
            }
            catch (dirCtxError) {
                logger.warn(LogCategory.ANALYSIS_QUEUE, `[目录上下文] 预获取目录上下文失败（不影响文件分析）: ${filePath}`, dirCtxError);
            }
            // ========== 第二阶段：内容提取与文本转换 ==========
            this.updateItemStatus(item.id, 'analyzing', 10, undefined, { analysisStage: 2 });
            const magikaGroup = typeof magikaCategory === 'string' ? magikaCategory : magikaCategory?.group;
            // ========== 并行内容提取与文本转换 ==========
            timer.start('markitdownServerExtraction');
            const extractFileCategory = getFileCategory(effectiveVirtualPath);
            const isPlainTextOrCode = extractFileCategory === FileCategory.TEXT || extractFileCategory === FileCategory.CODE;
            // 0. 统一调用 Omni 原生引擎进行端到端内容与元数据提取（覆盖文档、图片、纯文本、代码、音视频、字体等所有格式并输出真实 Benchmark）
            const anydocStartTime = Date.now();
            const anydocResult = await anydocService
            ...
            const anydocTextBytes = Buffer.byteLength(anydocResult.content || existingBasicData?.content || '', 'utf8');
            let generatedThumbnailOutPath = undefined;
            // 1. 缩略图与目录上下文并行决策
            const [, dirContext, thumbPath] = await Promise.all([
                ...
                (async () => {
                    if (existingBasicData.thumbnailPath)
                        return existingBasicData.thumbnailPath;
                    if (isNativeImage) {
                        // 浏览器原生格式：直接引用原图
                    ...
                    if (isImage) {
                        // 非原生图片：原尺寸转码 WebP
                        try {
                            const thumbResult = await thumbnailService.getOrGenerateOriginalTranscodedImage(filePath, fileFingerprint, enhancedSmartName, rootWorkspaceDir.path);
                            return thumbResult.success ? thumbResult.relativePath : undefined;
                        }
                        catch (thumbErr) {
                            logger.warn(LogCategory.ANALYSIS_QUEUE, `[FileProcessor] 生成缩略图失败 (${item.name}):`, thumbErr);
                            return undefined;
                        }
                    }
                    ...
                    if (rootWorkspaceDir) {
                        try {
                            ...
                            const success = await thumbnailService.generateThumbnailFallback(filePath, outPath, String(existingWorkspaceFile?.id || ''));
                            if (success) {
                                return path.relative(rootWorkspaceDir.path, outPath);
                            }
                        }
                        catch (err) {
                            logger.debug(LogCategory.ANALYSIS_QUEUE, `[FileProcessor] Omni 封面缩略图生成跳过: ${err?.message || err}`);
                        }
                    }
                    return undefined;
                })()
            ...
            if (magikaCategory) {
                logger.info(LogCategory.ANALYSIS_QUEUE, `[FileProcessor] Magika 分类识别完成: ${item.name} (${typeof magikaCategory === 'string' ? magikaCategory : magikaCategory.label || magikaCategory.group})`);
            }
            ...
            if (isImage && combinedContent) {
                if (isGibberishOcrText(combinedContent)) {
                    logger.warn(LogCategory.ANALYSIS_QUEUE, `[FileProcessor] OCR 识别文本判定为视觉噪点/乱码，已自动丢弃: ${item.name}`);
                    combinedContent = '';
                }
            }
            ...
            if (!combinedContent.trim() && existingBasicData.content && !isImage) {
                ...
                logger.info(LogCategory.ANALYSIS_QUEUE, `[复用数据] 复用已有文本内容: ${item.name}, 长度: ${combinedContent.length}`);
            }
                ...
                try {
                    ...
                    if (textProcessor.canProcess(path.basename(filePath), enhancedFileType)) {
                        ...
                        if (textContent) {
                            ...
                            logger.info(LogCategory.ANALYSIS_QUEUE, `[FileProcessor] TextFileProcessor 降级提取文本内容成功: ${item.name}, 长度: ${textContent.length}`);
                        }
                    }
                }
                catch (fallbackError) {
                    logger.warn(LogCategory.ANALYSIS_QUEUE, `[FileProcessor] TextFileProcessor 降级提取失败: ${item.name}`, fallbackError);
                }
            }
            // ========== 阶段 1 细分耗时整合 ==========
            const stage1TotalMs = (typeof timer?.getPhases === 'function'
                ? timer.getPhases()['hashAndTypeIdentification']
                : undefined) ?? (stage1FingerprintMs + stage1LocalReuseMs + stage1CloudReuseMs);
            const stage1Benchmark = {
            ...
            const calculatedMaxParallelTotalMs = Math.max(stage2MagikaMs || 0, stage2MetadataMs || 0, stage2TagMs || 0, stage2TextMs || 0, stage2OcrMs || 0, stage2ThumbMs || 0);
            markitdownBenchmark = {
            ...
            const finalMaxKb = ConfigOrchestrator.getInstance().getValue('MAX_CONTENT_SIZE_KB') ?? 1024;
            if (finalMaxKb > 0) {
                ...
                if (currentContentBytes > maxBytes) {
                    logger.info(LogCategory.ANALYSIS_QUEUE, `[FileProcessor] 组合后文本总字节数(${currentContentBytes} B)超出配置上限，统一按 MAX_CONTENT_SIZE_KB (${finalMaxKb}KB = ${maxBytes} B) 进行 UTF-8 字符边界防乱码安全截断`);
            ...
            logger.info(LogCategory.ANALYSIS_QUEUE, `[FileProcessor] 阶段2内容与元数据提取完成: file=${item.name} combinedContentLen=${combinedContent.length} (metadataKeys=${Object.keys(contentResult.metadata || {}).length})`);
            // 处理缩略图路径
            if (thumbPath) {
                thumbnailRelativePath = thumbPath;
            }
            else if (generatedThumbnailOutPath &&
        ...
        `).run(fileFingerprint, fileInfo.name, stats.size, enhancedFileType, JSON.stringify(magikaCategory || { mime_type: getMimeType(enhancedFileType) }), omniDescription || null, new Date(stats.birthtime).toISOString(), new Date(stats.mtime).toISOString(), new Date(stats.atime).toISOString());
            ...
            try {
                ...
                const isImageOrMedia = enhancedFileType === 'image' ||
                    (magikaCategory?.mime_type && magikaCategory.mime_type.startsWith('image/'));
                const ocrOrExtractedText = (isImageOrMedia && contentResult.content) ||
          ...
          `).run(fileFingerprint, contentResult.content ?? null, JSON.stringify(contentResult.metadata || {}), finalLrc);
            ...
            ON CONFLICT(workspace_id, path) DO UPDATE SET
              ...
              modified_at = excluded.modified_at,
              accessed_at = ?
            `).run(fileFingerprint, currentWorkspaceId, directoryId, filePath, path.basename(filePath), new Date(stats.birthtime).toISOString(), new Date(stats.mtime).toISOString(), new Date(stats.atime).toISOString(), 0, new Date().toISOString());
                    const wsFileRow = db
                    ...
                    logger.warn(LogCategory.ANALYSIS_QUEUE, '[分析队列] 实时保存 CPU 阶段耗时失败:', saveCpuError);
                }
            ...
            if (analysisMode !== 'full' && analysisMode !== 'quick_name') {
                ...
                const { workspaceFile } = await saveLocalAnalysisResult(item, fileFingerprint, processResult, magikaCategory, enhancedSmartName, enhancedFileType, thumbnailRelativePath || undefined, currentWorkspaceId, timer, this.collectAnalysisStats.bind(this), true, undefined, undefined, markitdownBenchmark, 1, undefined, stage1Benchmark);
                logger.info(LogCategory.ANALYSIS_QUEUE, `[基础分析] 保存 category: ${JSON.stringify(magikaCategory).slice(0, 200)}`);
                ...
                const analysisStatsWithBenchmark = applyMarkitdownBenchmark(analysisStats, markitdownBenchmark);
            ...
            let dimResult;
            if (analysisMode === 'quick_name') {
                const quickRes = await processQuickNameAnalysis(item, fileFingerprint, fileInfo, thumbnailRelativePath || undefined, rootWorkspaceDir.path, timer, deps, {
                    language,
                    directoryContext,
                    magikaCategory,
                    isSpeedy,
                ...
                const fullRes = await processLocalAnalysis(item, fileFingerprint, fileInfo, thumbnailRelativePath || undefined, rootWorkspaceDir.path, timer, deps, {
                    language,
                    directoryContext,
                    magikaCategory,
                    isSpeedy,
            ...
            this.updateItemStatus(item.id, 'analyzing', 98);
            // ========== 保存本地分析结果 ==========
            const { workspaceFile } = await saveLocalAnalysisResult(item, fileFingerprint, processResult, magikaCategory, dimResult?.smartName || enhancedSmartName, enhancedFileType, thumbnailRelativePath || undefined, currentWorkspaceId, timer, this.collectAnalysisStats.bind(this), false, dimResult?.groupingReason, dimResult?.groupingConfidence, markitdownBenchmark, analysisMode === 'quick_name' ? 3 : 4, cpuSkipped, stage1Benchmark);
            ...
            if (!finalMergedStats) {
                const analysisStats = await this.collectAnalysisStats(timer);
                finalMergedStats = applyMarkitdownBenchmark(analysisStats, markitdownBenchmark, stage1Benchmark);
            }
            timer.printSummary();
            this.updateItemStatus(item.id, 'completed', 100, undefined, {
                analysisStats: finalMergedStats,
            ...
            if (isAbort) {
                logger.info(LogCategory.ANALYSIS_QUEUE, `[分析队列] 分析任务被中止，恢复状态为 pending: ${item.name}`);
                this.updateItemStatus(item.id, 'pending', 0);
            }
            else {
                let errorMsg = error instanceof Error ? error.message : String(error);
                if (errorMsg.includes('timeout') || errorMsg.includes('超时')) {
                    if (errorMsg.includes('元数据') ||
     ...
     */
    async saveBasicAuthorTags(fileFingerprint, metadata, db) {
        try {
            const result = saveAuthorTagsFromMetadata(db, fileFingerprint, metadata, 4);
            if (result.authorNames.length > 0) {
                db.prepare('UPDATE files SET author = ? WHERE file_fingerprint = ?').run(result.authorValue, fileFingerprint);
                logger.info(LogCategory.ANALYSIS_QUEUE, `[基础分析] 已从元数据提取作者标签: ${result.authorNames.join(', ')}`);
            }
        }
        catch (error) {
     ...
     */
    async saveBasicLanguageTags(fileFingerprint, metadata, db) {
        try {
            const result = saveLanguageTagsFromMetadata(db, fileFingerprint, metadata, 4);
            if (result.languageValue) {
                db.prepare('UPDATE files SET language = ? WHERE file_fingerprint = ?').run(result.languageValue, fileFingerprint);
                logger.info(LogCategory.ANALYSIS_QUEUE, `[基础分析] 已从元数据提取语言标签: ${result.languageValue}`);
            }
        }
        catch (error) {
     ...
     */
    sanitizeMagikaCategory(magikaCategory, filePath) {
        if (!magikaCategory || typeof magikaCategory === 'string')
            return magikaCategory;
     ...
     */
    saveBasicMagikaTags(fileFingerprint, magikaCategory, filePath, db) {
        try {
            const isMagikaReliable = magikaCategory &&
            ...
            if (isMagikaReliable) {
                ...
                if (originalExt) {
                    // 保存扩展名标签
                    saveExtensionTags(db, fileFingerprint, { extensions: [originalExt] }, 4);
                    // 从扩展名反推 group 并保存类型标签
                    const group = getMagikaGroupFromExtension(originalExt);
                    if (group) {
                        saveMagikaGroupTag(db, fileFingerprint, { group }, 4);
                    }
                }
            }
        }
     ...
     * 保存云端分析结果到数据库
     */
    async saveCloudResultToDB(item, fileFingerprint, data, isCloudCache, workspaceId) {
        return saveCloudResult(item, fileFingerprint, data, isCloudCache, workspaceId, this.getModelName.bind(this));
    }
    /**
     * 处理空文件
     */
    async handleEmptyFile(item, workspaceId) {
        return handleEmptyFile(item, workspaceId);
    }
    /**
     * Helper to compute the final file type and smart name based on Magika category
     */
    getEnhancedFileInfo(originalName, originalType, filePath, magikaCategory) {
        ...
        if (!diskExt && magikaCategory && typeof magikaCategory !== 'string') {
            ...
            const isMagikaReliable = magikaCategory.group &&
                magikaCategory.group !== 'unknown' &&
                (magikaCategory.score ?? 1) >= 0.6;
            if (isMagikaReliable) {
                const magikaExt = magikaCategory.extensions && magikaCategory.extensions.length > 0
    ...
    async applyMockResult(itemId, filePath, workspaceId) {
        const mockJsonPath = process.env.TEST_MOCK_JSON_PATH;
        ...
        try {
            ...
            const mockWorkspaceFile = this.mockData.workspace_files.find((f) => f.name &&
                f.name.toLowerCase() === fileName.toLowerCase() &&
                (f.is_analyzed === 1 || f.is_analyzed === true));
            if (!mockWorkspaceFile)
                return false;
            const fingerprint = mockWorkspaceFile.file_fingerprint;
            const fileData = this.mockData.files.find((f) => f.file_fingerprint === fingerprint);
            if (!fileData)
                return false;
            const db = databaseService.db;
      ...
      `).run(fingerprint, fileData.smart_name || fileName, fileData.description || '', fileData.size || 0, fileData.type || 'unknown', fileData.category ? JSON.stringify(fileData.category) : null, fileData.author || '', fileData.language || 'zh-CN', now, now, now);
            db.prepare(`
      ...
      `).run(fingerprint, workspaceId, directoryId, filePath, fileName, now, now, now, now, now, now);
            if (this.mockData.file_tag_relations && this.mockData.file_tags) {
                ...
                const relations = this.mockData.file_tag_relations.filter((r) => r.file_fingerprint === fingerprint);
                for (const rel of relations) {
                    const tag = this.mockData.file_tags.find((t) => t.id === rel.tag_id);
                    if (tag) {
                        db.prepare('INSERT OR IGNORE INTO file_tags (name, dimension_id) VALUES (?, ?)').run(tag.name, tag.dimension_id);
                        const localTagRow = db
                            .prepare('SELECT id FROM file_tags WHERE name = ? AND dimension_id = ?')
                            .get(tag.name, tag.dimension_id);
                        if (localTagRow) {
                            db.prepare(`INSERT OR IGNORE INTO file_tag_relations (file_fingerprint, tag_id, sync_status) VALUES (?, ?, 0)`).run(fingerprint, localTagRow.id);
                        }
                    }
                }
            }
 ...
 * 从元数据中提取候选歌词
 */
export function getFallbackLyrics(metadata) {
    if (!metadata)
        return null;
 ...
 */
export function safeTruncateUtf8Bytes(str, maxBytes) {
    if (!str || maxBytes <= 0)
        return '';
