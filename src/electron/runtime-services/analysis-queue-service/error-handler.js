...
export class ErrorHandler {
     ...
     * 记录分析错误（增强版）
     */
    logAnalysisError(filePath, error, errorType = AnalysisErrorType.UNKNOWN_ERROR, retryCount = 0, _context) {
        const analysisError = {
    ...
    getRecoveryAction(errorType) {
        switch (errorType) {
            ...
            case AnalysisErrorType.AI_REQUEST_TIMEOUT: {
                const configMode = ConfigOrchestrator.getInstance().getValue('AI_SERVICE_MODE') || 'local';
                if (configMode === 'cloud') {
                    return t('检查AI服务状态或增加超时时间，建议切换云端模型或服务商');
                }
                return t('检查AI服务状态或增加超时时间，建议切换低显存需求的AI模型');
            }
    ...
    classifyError(error, context) {
        ...
        if (message.includes('network') ||
            ...
            message.includes(t('网络')) ||
            message.includes(t('连接'))) {
            return AnalysisErrorType.AI_SERVICE_ERROR;
        }
        // 内存和资源错误
        if (message.includes('out of memory') ||
            message.includes(t('内存不足')) ||
            message.includes('resource exhausted') ||
            message.includes(t('资源耗尽'))) {
     ...
     * 检查是否应该重试
     */
    shouldRetry(errorType, retryCount, filePath) {
    ...
    isCriticalError(errorType) {
        const criticalErrors = [AnalysisErrorType.DATABASE_ERROR, AnalysisErrorType.PERMISSION_DENIED];
     ...
     * 获取错误恢复建议（增强版）
     */
    getEnhancedRecoveryAction(errorType, retryCount, filePath) {
    ...
    getErrorStatistics() {
        ...
        const recentErrors = this.errorHistory.filter(error => new Date(error.timestamp).getTime() > oneHourAgo);
        const errorsByType = {};
        ...
        const mostCommonError = Object.entries(errorsByType).sort(([, a], [, b]) => b - a)[0]?.[0] ||
            null;
        return {
