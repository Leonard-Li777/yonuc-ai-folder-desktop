...
export class OmniService {
    static instance;
    process = null;
    basePort = APP_PORTS.OMNI_SERVER;
    actualPort = APP_PORTS.OMNI_SERVER;
    baseUrl = `http://127.0.0.1:${APP_PORTS.OMNI_SERVER}`;
    isStarting = false;
    startPromise = null;
    cachedVersion = null;
    restartAttempts = 0;
    ...
    constructor() {
        try {
            ...
            import('../../config/config-orchestrator').then(({ ConfigOrchestrator }) => {
                ...
                ocrKeys.forEach(key => {
                    orchestrator.onValueChange(key, () => {
    ...
    async doStart() {
        ...
        try {
            ...
            const isCzkawkaDump = (str) => {
                return (str.includes('DEBUG PRINT COMMON') ||
     ...
     */
    async syncConfigFromDesktop() {
        try {
            const { ConfigOrchestrator } = await import('../../config/config-orchestrator');
            const orchestrator = ConfigOrchestrator.getInstance();
            const enableImageOcr = orchestrator.getValue('ENABLE_IMAGE_OCR') ?? true;
            const enableOfficeCover = orchestrator.getValue('ENABLE_OFFICE_COVER') ?? false;
            const maxDocOcrItems = orchestrator.getValue('MAX_DOCUMENT_OCR_ITEMS') ?? 0;
            const ocrModelSize = (orchestrator.getValue('OCR_MODEL_SIZE') || 'tiny').toLowerCase();
            const maxContentSizeKb = orchestrator.getValue('MAX_CONTENT_SIZE_KB') ?? 30;
            const maxFileSizeMb = orchestrator.getValue('MAX_FILE_SIZE') ?? 100;
            const analysisMode = (orchestrator.getValue('ANALYSIS_MODE') || 'full').toLowerCase();
            const reuseBasic = orchestrator.getValue('REUSE_BASIC_ANALYSIS_DATA') ?? true;
            ...
            if (res.ok) {
                logger.info(LogCategory.SYSTEM, `[OmniService] 已向 Omni 引擎同步配置: enable_office_cover=${enableOfficeCover}, max_document_ocr_items=${maxDocOcrItems}, enable_image_ocr=${enableImageOcr}, ocr_model_size=${ocrModelSize}, excluded_items_count=${excludedItems.length}`);
                return true;
            }
            return false;
        }
    ...
    async extract(filePath) {
        ...
        logger.debug(LogCategory.SYSTEM, `[OmniService] >>> POST /api/extract 请求发起:`, JSON.stringify(reqBody));
        const doFetch = async () => {
            ...
            if (!res.ok) {
                logger.warn(LogCategory.SYSTEM, `[OmniService] <<< POST /api/extract 响应异常: status=${res.status}, 耗时: ${Date.now() - tStart}ms`);
                return null;
            }
            const json = (await res.json());
            logger.debug(LogCategory.SYSTEM, `[OmniService] <<< POST /api/extract 响应成功 (${filePath}, 耗时: ${Date.now() - tStart}ms):`, JSON.stringify(json));
            return json;
        };
        try {
            ...
            logger.warn(LogCategory.SYSTEM, `[OmniService] <<< POST /api/extract 首次调用异常 (${filePath}): 耗时: ${Date.now() - tStart}ms, 错误: ${err.message}，尝试自愈拉起并重试...`);
            ...
            if (restarted) {
                try {
                    ...
                    logger.error(LogCategory.SYSTEM, `[OmniService] <<< POST /api/extract 自愈重试仍失败 (${filePath}): ${retryErr.message}`);
                }
            }
            return null;
        }
     ...
     */
    async extractMetadataFull(filePath) {
     ...
     */
    async reverseGeo(pointsOrLat, lonOrLang, optionalLang) {
        await this.ensureRunning();
        const tStart = Date.now();
        try {
            let points = [];
            ...
            if (typeof pointsOrLat === 'number') {
                ...
                points = pointsOrLat.map(p => ({
                    latitude: 'latitude' in p ? p.latitude : p.lat,
                    longitude: 'longitude' in p ? p.longitude : p.lon
                }));
            }
            if (points.length === 0)
                return null;
            ...
            logger.debug(LogCategory.SYSTEM, `[OmniService] >>> POST /api/geo/reverse 请求发起:`, JSON.stringify(reqBody));
            const doFetch = async () => {
                ...
                if (!res.ok) {
                    logger.warn(LogCategory.SYSTEM, `[OmniService] <<< POST /api/geo/reverse 响应异常: status=${res.status}, 耗时: ${Date.now() - tStart}ms`);
                    return null;
                }
                const json = (await res.json());
                logger.debug(LogCategory.SYSTEM, `[OmniService] <<< POST /api/geo/reverse 响应成功 (耗时: ${Date.now() - tStart}ms):`, JSON.stringify(json));
                return json;
            };
            try {
                return await doFetch();
    ...
    async getFileCover(filePath) {
        ...
        const doFetch = async () => {
            ...
            if (res.status === 204 || !res.ok) {
                logger.debug(LogCategory.SYSTEM, `[OmniService] <<< GET /api/cover 响应 ${res.status} (未开启封面或不支持, 耗时: ${Date.now() - tStart}ms, ${filePath})`);
                return null;
            }
            const arrayBuffer = await res.arrayBuffer();
            const buffer = Buffer.from(arrayBuffer);
            logger.debug(LogCategory.SYSTEM, `[OmniService] <<< GET /api/cover 响应成功 (${filePath}, 字节大小: ${buffer.length} B, 耗时: ${Date.now() - tStart}ms)`);
            return buffer;
        };
        try {
            return await doFetch();
    ...
    async perceive(filePath, options) {
        ...
        const doFetch = async () => {
            ...
            if (!res.ok) {
                logger.warn(LogCategory.SYSTEM, `[OmniService] <<< POST /api/perceive 响应异常: status=${res.status}, 耗时: ${Date.now() - tStart}ms`);
                return null;
            }
            const json = (await res.json());
            ...
            if (json.winning_hypothesis || (json.candidate_hypotheses && json.candidate_hypotheses.length > 0)) {
                logger.debug(LogCategory.SYSTEM, `[级联仲裁:Desktop] 终局胜出: "${json.winning_hypothesis?.prompt_text || '无'}" (置信度: ${json.winning_hypothesis?.confidence ?? 'N/A'}) | 建议命名: ${json.smart_name || '无'}`);
                if (json.candidate_hypotheses && json.candidate_hypotheses.length > 0) {
                    for (const cand of json.candidate_hypotheses) {
                        logger.debug(LogCategory.SYSTEM, `[级联仲裁:Desktop] 候选假设 id=${cand.id} winner=${cand.is_winner ? '★YES' : ' NO '} conf=${cand.confidence} prompt="${cand.prompt_text}"`);
                    }
                }
            }
            logger.debug(LogCategory.SYSTEM, `[OmniService] <<< POST /api/perceive 响应成功 (${filePath}, 耗时: ${Date.now() - tStart}ms):`, JSON.stringify({
        ...
        try {
            ...
            logger.warn(LogCategory.SYSTEM, `[OmniService] <<< POST /api/perceive 首次调用异常 (${filePath}): 耗时: ${Date.now() - tStart}ms, 错误: ${err.message}，尝试自愈拉起并重试...`);
            const restarted = await this.start();
            if (restarted) {
                try {
                    return await doFetch();
                    ...
                    logger.error(LogCategory.SYSTEM, `[OmniService] <<< POST /api/perceive 自愈重试仍失败 (${filePath}): ${retryErr.message}`);
                }
            }
            return null;
        }
     ...
     */
    async extractVisualTags(filePath, options) {
        await this.ensureRunning();
        const reqBody = {
            file_path: filePath,
            language: options?.language,
     ...
     */
    async inspectVision(filePath, timeoutMs = 10000) {
        await this.ensureRunning();
        const reqBody = { file_path: filePath };
        const doFetch = async () => {
     ...
     * POST /api/fs/ads
     */
    async inspectAds(filePath, timeoutMs = 5000) {
        await this.ensureRunning();
        const reqBody = { file_path: filePath };
        const doFetch = async () => {
