...
export class LoggingService {
    ...
    stableSerialize(obj) {
        ...
        if (Array.isArray(obj))
            return `[${obj.map(v => this.stableSerialize(v)).join(',')}]`;
        const keys = Object.keys(obj).sort();
        const entries = keys.map(k => `"${k}":${this.stableSerialize(obj[k])}`);
        return `{${entries.join(',')}}`;
    }
    makeSuppressionKey(entry) {
     ...
     */
    logAppError(error) {
        this.error(LogCategory.ERROR, error.code, {
     ...
     * 记录日志
     */
    log(level, category, message, data, stack) {
     ...
     */
    formatConsolePrefix(logEntry, timestamp, levelName) {
        const reset = '\x1b[0m';
        const levelColor = logEntry.level === LogLevel.ERROR
            ? '\x1b[31m'
            : logEntry.level === LogLevel.WARN
                ...
                : logEntry.level === LogLevel.DEBUG
                    ? '\x1b[90m'
                    : '\x1b[32m';
        const categoryColor = LogCategoryColors[logEntry.category] || '\x1b[37m';
     ...
     * 创建应用错误对象
     */
    createAppError(type, code, message, details, recoverable = true, context) {
        return {
            type,
            code,
            message,
     ...
     * 包装异步函数以添加错误处理和日志记录
     */
    wrapAsyncFunction(fn, category, context) {
        return fn().catch(error => {
            const appError = this.createAppError(ErrorType.SYSTEM, 'ASYNC_FUNCTION_ERROR', error instanceof Error ? error.message : String(error), error, true, context);
     ...
     * 包装同步函数以添加错误处理和日志记录
     */
    wrapSyncFunction(fn, category, context) {
        try {
            return fn();
        }
        catch (error) {
            const appError = this.createAppError(ErrorType.SYSTEM, 'SYNC_FUNCTION_ERROR', error instanceof Error ? error.message : String(error), error, true, context);
            this.logAppError(appError);
            throw error;
        }
    }
