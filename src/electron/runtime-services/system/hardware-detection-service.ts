...
export class HardwareDetectionService implements IHardwareDetectionService {
  ...
  async detectSystemResources(): Promise<ISystemResources> {
    ...
    // 获取 Ollama 模型存储路径
    const ollamaModelsPath = OllamaService.getInstance().getOllamaModelsPath();

    const [cpu, memory, gpus, storage] = await Promise.all([
      this.detectCPU(),
      this.detectMemory(),
      this.detectGPUs(),
      this.detectStorage(ollamaModelsPath)
    ]);
  ...
  /**
   * 检测存储信息
   * @param targetPath 可选的目标路径，如果指定则只返回该路径所在磁盘的信息
   */
  private async detectStorage(targetPath?: string): Promise<ISystemResources['storage']> {
    try {
      ...
      if (platform === 'win32') {
        ...
        // 如果指定了 targetPath，解析出驱动器盘符 (例如 "C:")
        let targetDrive = '';
        if (targetPath) {
          const parsed = path.parse(targetPath);
          if (parsed.root && parsed.root.includes(':')) {
            // path.parse('C:\Users').root -> 'C:\'
            targetDrive = parsed.root.replace(/[\\/]/g, '').toUpperCase(); // "C:"
          }
        }

        let stdout = '';
        // 增加 DeviceID 以便筛选
        const psCommand = 'powershell -ExecutionPolicy Bypass -Command "Get-CimInstance Win32_LogicalDisk | Select-Object DeviceID, Size, FreeSpace | ConvertTo-Csv -NoTypeInformation"';
        try {
          const result = await execAsync(psCommand);
          stdout = result.stdout;
        } catch (e) {
          logger.warn(LogCategory.HARDWARE_DETECTION, 'PowerShell 磁盘检测失败，尝试回退 wmic', e);
          const wmicResult = await execAsync('wmic logicaldisk get DeviceID,Size,FreeSpace /format:csv');
        ...
        for (const line of lines) {
          ...
          if (parts.length >= 3) {
            ...
            // 尝试推断字段位置
            // PowerShell: DeviceID, Size, FreeSpace
            // wmic (CSV): Node, DeviceID, FreeSpace, Size (按字母顺序排序: DeviceID, FreeSpace, Size)
            
            // 简单启发式：查找带冒号的作为 DeviceID
            const driveIndex = parts.findIndex(p => p.includes(':'));
            
            if (driveIndex !== -1) {
              deviceId = parts[driveIndex].toUpperCase();
              
              // 剩下的数字中，大的通常是 Total，小的是 Free
              const numbers = parts.filter((_, i) => i !== driveIndex && !isNaN(parseInt(_))).map(n => parseInt(n));
              if (numbers.length >= 2) {
                 // 排序：小 -> 大
                 numbers.sort((a, b) => a - b);
                 freeSpace = numbers[0]; // 较小的是可用空间
                 totalSpace = numbers[numbers.length - 1]; // 最大的是总空间
              }
            } else {
               // 无法识别盘符，尝试按位置解析 (假设是 PowerShell 顺序)
               if (parts.length === 3) {
                  deviceId = parts[0];
        ...
        // Unix系统: 使用df命令
        // 如果指定了路径，直接检测该路径
        const cmd = targetPath ? `df -B1 "${targetPath}"` : 'df -B1 /';
        ...
        try {
          ...
          if (lines.length > 1) {
            // df 输出通常是: Filesystem 1B-blocks Used Available Use% Mounted on
            // 获取最后一行（防止有多行输出）
            const lastLine = lines.filter(l => l.trim().length > 0).pop();
            if (lastLine) {
              const parts = lastLine.split(/\s+/);
              if (parts.length >= 4) {
                // 索引: 0=Filesystem, 1=Size, 2=Used, 3=Available
           ...
           // 如果指定路径失败（例如目录不存在），尝试回退到根目录
           if (targetPath) {
             logger.warn(LogCategory.HARDWARE_DETECTION, `检测路径 ${targetPath} 失败，回退到根目录`, error);
