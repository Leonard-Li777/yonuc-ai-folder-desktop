...
import { LogCategory, logger, verifyLicense, encodeMachineIdToRef, encodeMachineIdToBase64, isTestEnvironment } from '@firefly/shared';
...
(function (LicenseStatus) {
    LicenseStatus["AUTHORIZED"] = "AUTHORIZED";
    LicenseStatus["UNAUTHORIZED"] = "UNAUTHORIZED";
    LicenseStatus["EXPIRED"] = "EXPIRED";
    LicenseStatus["TIME_TAMPERED"] = "TIME_TAMPERED";
    LicenseStatus["PENDING_ONLINE"] = "PENDING_ONLINE";
...
export class LicenseService {
    ...
    licenseFilePath;
    publicKey;
    isOnlineAuthorized = false; // 内存中的在线授权状态
    cachedResult = null; // 缓存的授权状态
    hmacKey = null; // HMAC 密钥，启动时派生一次
    lastForceCheckTime = 0; // 上次强制检查的时间戳
    constructor() {
    ...
    verifyTimeHmac(key) {
        ...
        const value = config.getValue(key) || 0;
        const storedHmac = config.getValue(`${key}_HMAC`);
        // 默认值（0）无需验证
    ...
    checkServerConnectivityGrace() {
        ...
        const lastServerConnect = config.getValue('LAST_SERVER_CONNECT_TIME') || 0;
        const lastRun = config.getValue('LAST_RUN_TIME') || 0;
        console.log({
            lastServerConnect,
            lastRun,
        ...
        if (currentTime - lastServerConnect > GRACE_PERIOD_MS) {
            ...
            if (lastRun > 0 && currentTime - lastRun < GRACE_PERIOD_MS) {
                logger.warn(LogCategory.SYSTEM, '[License] Grace 检查触发：服务器超过 3 天不可达，但应用在运行');
                return false;
            }
        }
        return true;
    ...
    async checkRealConnectivity() {
        ...
        if (regionResult) {
            ...
            logger.debug(LogCategory.SYSTEM, `[License] 使用 RegionDetectionService 的探测结果: ${isActuallyOnline}`);
            if (isActuallyOnline)
                return true;
        }
        ...
        const mirror = orchestrator.getValue('DOWNLOAD_MIRROR') || 'cn';
     ...
     */
    async checkOfflineLicense() {
        const config = ConfigOrchestrator.getInstance();
        let licenseStr = config.getValue('OFFLINE_LICENSE');
     ...
     */
    async writeEmbeddedUserTierData(userTierData, licenseCode, machineId) {
        const config = ConfigOrchestrator.getInstance();
     ...
     */
    async restoreFromOfflineLicenseIfNeeded() {
        const config = ConfigOrchestrator.getInstance();
        let licenseStr = config.getValue('OFFLINE_LICENSE');
        if (!licenseStr && fs.existsSync(this.licenseFilePath)) {
            licenseStr = fs.readFileSync(this.licenseFilePath, 'utf8');
        ...
        try {
            ...
            if (!data.userTierData)
                return false;
            await this.writeEmbeddedUserTierData(data.userTierData, licenseStr, machineId);
            return true;
        }
        catch {
            return false;
    ...
    async activate(licenseCode) {
        ...
        if (!result.data.ids.includes(machineId) &&
        ...
        if (isEnterpriseFormat) {
            // 新版企业授权码：直接使用内嵌的数据
            const enterpriseData = result.data;
            const userTierData = enterpriseData.userTierData;
        ...
        try {
            ...
            if (isEnterpriseFormat) {
                const enterpriseData = result.data;
                await this.writeEmbeddedUserTierData(enterpriseData.userTierData, licenseCode, machineId);
            }
            else {
     ...
     */
    initializeGraceTimestamps() {
        const config = ConfigOrchestrator.getInstance();
        const lastServerConnect = config.getValue('LAST_SERVER_CONNECT_TIME') || 0;
        // 首次安装：LAST_SERVER_CONNECT_TIME 为 0，设置为当前时间
        if (lastServerConnect === 0) {
            logger.info(LogCategory.SYSTEM, '[License] 首次安装，初始化 LAST_SERVER_CONNECT_TIME 为当前时间');
            this.updateTimeWithHmac('LAST_SERVER_CONNECT_TIME', Date.now());
        }
        // 验证 LAST_RUN_TIME 的 HMAC
        if (!this.verifyTimeHmac('LAST_RUN_TIME')) {
        ...
        // 验证 LAST_SERVER_CONNECT_TIME 的 HMAC
        if (!this.verifyTimeHmac('LAST_SERVER_CONNECT_TIME')) {
            logger.warn(LogCategory.SYSTEM, '[License] LAST_SERVER_CONNECT_TIME HMAC 验证失败，重置为当前时间');
            this.updateTimeWithHmac('LAST_SERVER_CONNECT_TIME', Date.now());
        }
    }
    /**
     * 检查在线授权状态
     */
    setOnlineAuthorized(authorized) {
